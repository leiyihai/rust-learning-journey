...
- `src/bin/ex01_hello.rs` ... `ex20_todo_app.rs` — 20 个独立可执行练习，每个 `[[bin]]` 在 Cargo.toml 中声明
- `src/bin/ex09_extra.rs` — 部分课程有附加练习题
- `src/` — 练习抽取的模块文件放在这里（非 `src/bin/`，避免被当成二进制）
- `lessons/` — 每课配套讲义（Markdown），用户先读讲义再做练习
- `PROGRESS.md` — 进度追踪表，`[ ]` 改 `[x]` 标记完成
- `NOTES.md` — 用户个人踩坑日志，记录学习中的错误和心得
- `Cargo.toml` — 无外部依赖，纯标准库教学；新增 bin 需同时注册 `[[bin]]`
...
- **未完成练习**：答案应默认注释掉或为 `todo!()` 占位，IDLE 打开时是题目状态
- **已完成练习**：用户自行取消注释并补全代码，TODO 注释保留在原处
- 新增练习需注册到 `Cargo.toml` 的 `[[bin]]` 表
- 练习配套的模块文件（如 `math_utils.rs`）放在 `src/` 而非 `src/bin/`，否则 Cargo 会要求它有 `main()`
- 跨目录引用模块用 `#[path = "../xxx.rs"]` 标注路径

## 用户偏好

- 交流使用中文，仅代码块保留英文原文
- 用户有 C# 背景，可用 C# 概念作为类比
- 知识盲区（如栈/堆、内存结构）需拆解成通俗比喻解释
- 用户会主动提出"这部分不理解，请举例"，此时给具体例子而非继续讲理论
