...
fn main() {
    ...
    let copy = original.clone();  // 填上正确的方法名
    ...
    //i32实现了Copy trait， String没有实现Copy trait遵守所有权规则，不克隆就所有权转移到copy 打印就会报错因为失效了
...
// 避免悬垂指针(不懂，只是知道)、避免数据竞争
// 2. 哪些类型实现了 Copy trait？哪些没有？规律是什么？
// 整数类型 bool char 元组(如果元组内所有元素都有Copy trait)实现了Copy trait
// 规律是看类型的内存分配  栈类型的都实现了Copy trait  堆类型的没有
// 3. 如果把一个 String 传给函数后又想继续使用它，有哪几种办法？
// 不使用借用的情况下，我只能想到这个
// let s2 = String::from("Ray");
// let s2 = SayHello(s2);
// fn SayHello(str: String) -> String {
//     println!("Hello,{}!", str);
//     str
// }
//如果函数本身也要返回值  那返回类型就用元组
// let s2 = String::from("Ray");
// let s2 = SayHello(s2).0;
// fn SayHello(str: String) -> tup:(String,String) {
